...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{a_numeric_vector <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{3}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{-2}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{5}\NormalTok{, }
    \DecValTok{5}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{1}\NormalTok{)}
...
\begin{frame}[fragile]{R: Some syntax}
\protect\hypertarget{r-some-syntax}{}
...
\begin{frame}[fragile]{R: Some syntax}
\protect\hypertarget{r-some-syntax-1}{}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# allowed but not preferred}
\NormalTok{a.variable =}\StringTok{ }\DecValTok{10} \OperatorTok{+}\StringTok{ }\DecValTok{1}
\CommentTok{## dots have 2 meanings in R, with a 3rd}
\CommentTok{## in the tidyverse}
...
\begin{frame}{R: Some reservations}
\protect\hypertarget{r-some-reservations}{}

\begin{itemize}
\tightlist
\item
  c, q, t, C, D, F, I, and T (via
...
\begin{frame}{R cheat sheets}
\protect\hypertarget{r-cheat-sheets}{}

\begin{itemize}
\tightlist
\item
  R to Matlab:
  \url{https://cran.r-project.org/doc/contrib/Hiebeler-matlabR.pdf}
\item
  R for programmers:
  \url{https://www.johndcook.com/blog/r_language_for_programmers/}
\item
  RStudio's giant list:
...
\protect\hypertarget{environment-variables}{}
...
\CommentTok{# to install from a git (requires the}
\CommentTok{# devtools package)}
...
\begin{frame}{ADNI \& ADNIMERGE}
\protect\hypertarget{adni-adnimerge}{}

\end{frame}

\begin{frame}[fragile]{Cleaning: Base}
\protect\hypertarget{cleaning-base}{}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(ADNIMERGE)}
\NormalTok{adni.cols <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{'RID'}\NormalTok{, }\StringTok{'VISCODE'}\NormalTok{, }\StringTok{'DX'}\NormalTok{, }\StringTok{'AGE'}\NormalTok{, }\StringTok{'PTGENDER'}\NormalTok{, }\StringTok{'PTEDUCAT'}\NormalTok{, }\StringTok{'PTETHCAT'}\NormalTok{, }\StringTok{'PTRACCAT'}\NormalTok{, }\StringTok{'APOE4'}\NormalTok{, }\StringTok{'FDG'}\NormalTok{, }\StringTok{'AV45'}\NormalTok{, }\StringTok{'CDRSB'}\NormalTok{, }\StringTok{'ADAS13'}\NormalTok{, }\StringTok{'MOCA'}\NormalTok{, }\StringTok{'WholeBrain'}\NormalTok{, }\StringTok{'Hippocampus'}\NormalTok{, }\StringTok{'MidTemp'}\NormalTok{, }\StringTok{'mPACCtrailsB'}\NormalTok{)}
\NormalTok{adni.rows <-}\StringTok{ }\KeywordTok{c}\NormalTok{(adnimerge}\OperatorTok{$}\NormalTok{VISCODE}\OperatorTok{==}\StringTok{'bl'} \OperatorTok{&}\StringTok{ }\NormalTok{adnimerge}\OperatorTok{$}\NormalTok{MOCA}\OperatorTok{>=}\DecValTok{16}\NormalTok{)}
\NormalTok{amerge_subset <-}\StringTok{ }\NormalTok{adnimerge[adni.rows,adni.cols]}
\NormalTok{amerge_subset <-}\StringTok{ }\NormalTok{amerge_subset[}\KeywordTok{complete.cases}\NormalTok{(amerge_subset),]}
\NormalTok{amerge_subset}\OperatorTok{$}\NormalTok{HMSCORE <-}\StringTok{ }\NormalTok{modhach}\OperatorTok{$}\NormalTok{HMSCORE[}\KeywordTok{match}\NormalTok{(amerge_subset}\OperatorTok{$}\NormalTok{RID, modhach}\OperatorTok{$}\NormalTok{RID)]}
\end{Highlighting}
\end{Shaded}

\end{frame}

\begin{frame}[fragile]{Cleaning: tidyverse}
\protect\hypertarget{cleaning-tidyverse}{}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(ADNIMERGE)}
\NormalTok{adnimerge }\OperatorTok{%>%}
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{select}\NormalTok{(RID, VISCODE, DX, AGE, PTGENDER, PTEDUCAT, PTETHCAT, PTRACCAT, APOE4, FDG, AV45, CDRSB, ADAS13, MOCA, WholeBrain, Hippocampus, MidTemp, mPACCtrailsB) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{filter}\NormalTok{(VISCODE }\OperatorTok{==}\StringTok{ "bl"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{filter}\NormalTok{(MOCA }\OperatorTok{>=}\StringTok{ }\DecValTok{16}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{drop_na}\NormalTok{() ->}\StringTok{ }\NormalTok{amerge_subset}

\NormalTok{amerge_subset }\OperatorTok{%<>%}\StringTok{ }\KeywordTok{inner_join}\NormalTok{(modhach[,}\KeywordTok{c}\NormalTok{(}\StringTok{"RID"}\NormalTok{,}\StringTok{"HMSCORE"}\NormalTok{)])}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{part-3-r-rmarkdown-more}{%
\section{Part 3: R, RMarkdown, \& more}\label{part-3-r-rmarkdown-more}}
