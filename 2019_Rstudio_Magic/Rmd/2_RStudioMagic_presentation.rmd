...
* `tidyverse` is a collection of R packages for transforming and visualizing data
* Core packages:
* `ggplot2`, `dplyr`, `tidyr`, `readr`, `tibble`, `stringr`, `purrr`, `forcats`
* Get them all with `install.packages("tidyverse")`
* Piping `%>%` to create a sequence of actions
...
* For a comparison of base and tidy code see scripts `/R/0_create_ADNI_data_base.R` and `1_create_ADNI_data_tidy.R`
